...
GROUP BY tr.Id,tispe.product_id""" % (pg_cs_task, time_selection, status_not_in)
...
%s
AND tr.`status` NOT IN (%s)
ORDER BY tr.Id,tq.qindex""" % (pg_cs_task, time_selection, status_not_in)
...
%s
AND tr.`status` NOT IN (%s)
GROUP BY tr.Id,tq.qindex""" % (pg_cs_task, time_selection, status_not_in)
#
# sql_delete_report = """DELETE FROM t_pg_report_mm WHERE taskid IN (%s)""" % pg_cs_task
#
# sql_delete_sku = """DELETE FROM t_pg_report_mm_sku_details WHERE taskid IN (%s)""" % pg_cs_task
#
...
def main():
    ...
    if main_data:
        ...
        with MySQLInstance(**mysql_db_ppzck_task, dict_result=True) as db:
            answer_main_df = DataFrame(db.query(sql_get_answer))
        ...
        for rid in rid_series.values:
            ...
            answer_df = answer_main_df[answer_main_df['rid'] == rid].copy()
