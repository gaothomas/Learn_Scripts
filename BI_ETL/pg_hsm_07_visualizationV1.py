...
FROM t_pg_report_hsm WHERE taskid IN (%s) AND check_sku = '1' AND category = '%s'"""
...
def main():
    ...
    for index1 in range(8):
        logger.info(sql_data % (task_new[index1], category[index1]))
        data.append(query_data_frame(mysql_db_bi_task, sql_data % (task_new[index1], category[index1])))
