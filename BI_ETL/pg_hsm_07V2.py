...
sql_delete_report = """DELETE FROM t_pg_report_hsm_%s WHERE taskid IN (%s)"""
...
with MySQLInstance(**mysql_db_bi_task, dict_result=True) as delete_db:
    for index1 in range(8):
        delete_db.execute(sql_delete_report % (category[index1], task_new[index1]))
        # delete_db.execute(sql_delete_sku % task_new[index1])
        delete_db.execute(sql_delete_report % (category[index1], task_old[index1]))
...
sql_insert_report_new = """INSERT INTO t_pg_report_hsm_%s (rid,addressIDnum,`month`,category,actual_sku,target_sku,
pg_display,total_display,pg_shelf,total_shelf,actual_fast_growing_sku,fast_growing_sku_compliance,base_sku,taskid,
category_num,check_all,check_sku) VALUES """

sql_insert_report_old = """INSERT INTO t_pg_report_hsm_%s (rid,addressIDnum,`month`,category,actual_sku,target_sku,
pg_display,total_display,pg_shelf,total_shelf,actual_fast_growing_sku,fast_growing_sku_compliance,base_sku,taskid,
category_num) VALUES """
...
def to_two_wave_result(i):
    ...
    sku_verification_df_new.drop_duplicates(subset='rid', inplace=True)
    sku_df_old.drop_duplicates(subset=['rid', 'product_id'], inplace=True)
    answer_df_old.drop_duplicates(subset=['rid', 'qid'], inplace=True)
    sku_verification_df_old.drop_duplicates(subset='rid', inplace=True)
    if i == 0:
        multi_option_df_new = query_data_frame(mysql_db_ppzck_task, sql_get_multi_option % ('共同陈列_', task_new[i]))
        multi_option_df_new.drop_duplicates(subset=['qid', 'option_index'], inplace=True)
        multi_option_df_old = query_data_frame(mysql_db_ppzck_task, sql_get_multi_option % ('共同陈列_', task_old[i]))
    ...
    new_df = pd.merge(new_df, store_new_df, how='inner', on='addressIDnum')
    old_df = pd.merge(old_df, store_old_df, how='inner', on='addressIDnum')
    ...
    new_df['check_sku'] = new_df.apply(lambda x: check_sku(x.sku_verification_count, x.check_pg_sku), axis=1)
    ...
    with MySQLInstance(**mysql_db_bi_task, dict_result=True) as db:
        db.executemany(sql_insert_report_new % category[i] + '(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
                       [tuple(x) for x in new_df.reindex(columns=insert_table_order_new).values])
        db.executemany(sql_insert_report_old % category[i] + '(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
